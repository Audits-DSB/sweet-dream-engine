...
- **Frontend**:
  - `Orders.tsx` (New Order dialog): bestSuppliers loaded via useEffect, auto-fill supplier/price, ⭐ best supplier badge, ↑↓ price diff arrows, price spike warning (>20% alert with cost difference), per-material supplier comparison card, order-level **profit-based** supplier comparison panel (sorted by highest profit, not lowest cost) with "التوزيع الأمثل" optimal mix option (best supplier per material) + per-supplier profit/margin/cost display, per-line supplier dropdown, supplier history dialog with price trend chart and full supply log
